...
import numpy as np
import torch
from PatchMixer.utils import compute_voxelization
...
CATEGORIES_COMMON = [
    ['dresser', 'wardrobe'], ['bench', 'chair', 'stool'], 'desk', 'monitor', 'door', 
    'bookshelf', 'table', 'bed', 'sink', 'sofa', 'toilet'
]

CATEGORIES_COMMON_PLOT_PointDA = [
    'bathtub', 'bed', 'bookshelf', ['dresser', 'wardrobe'], 'chair', 
    'lamp', 'monitor', 'plant', 'sofa', 'table'
]

CATEGORIES_COMMON_POINTDA_SONN = [
    'bed', 'bookshelf', ['dresser', 'wardrobe'], 'chair', 
    'monitor', 'sofa', 'table'
]
...
class dataset_mn40(torch.utils.data.Dataset):
    ...
    def __init__(self, args, split, transforms):
        ...
        self.categories_common = CATEGORIES_COMMON
...
class dataset_mn4010(dataset_mn40):
    def __init__(self, args, split, transforms):

        self.args = args
        self.split = split
        self.transforms = transforms
        self.categories = CATEGORIES_COMMON_PLOT_PointDA
        ...
        self.data = list()
        self.label = list()
        for id_common, category_common in enumerate(CATEGORIES_COMMON_PLOT_PointDA):
...
class dataset_mn407(dataset_mn40):
    def __init__(self, args, split, transforms):

        self.args = args
        self.split = split
        self.transforms = transforms
        self.categories = CATEGORIES_COMMON_POINTDA_SONN
        ...
        self.data = list()
        self.label = list()
        for id_common, category_common in enumerate(CATEGORIES_COMMON_POINTDA_SONN):
