...
def run_test(args, logger, checkpoint):
    ...
    for idx_batch, data_batch in enumerate(loader):
        ...
        # if target == mn40_categories_common_plot_pointDA.index('sofa') and tmp < 10 :
        #     tmp +=1
        #     fig = plt.figure(figsize=(16, 12), dpi=300)
        #     ax = fig.add_subplot(projection='3d')
        
        #     # item = (input[0][:, [0, 2, 1]], input[1], input[2])
        #     swapped_tensor = np.swapaxes(input, 1, 2)
            
        #     print(input)
        #     print(target)
        
        #     ax.scatter(
        #         swapped_tensor[:, :, 0].flatten(), swapped_tensor[:, :, 2].flatten(), swapped_tensor[:, :, 1].flatten(),
        ...
        #     plt.savefig(os.path.join(args.path_weights, 'plot', 'plot_{:s}_{:d}.png'.format(args.dataset_type, tmp)))
    ...
    sns.heatmap(conf_mat, annot=True, fmt="d", xticklabels=loader.dataset.categories, yticklabels=loader.dataset.categories)
    ...
    if args.flag_compute_tsne:
        ...
        #print("scatter elements", *scatter.legend_elements())
