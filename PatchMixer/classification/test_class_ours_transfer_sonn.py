...
from datasets_sonn import CATEGORIES as sonn_categories
from datasets_sonn import CATEGORIES_COMMON as sonn_categories_common
from datasets_sonn import CATEGORIES_COMMON_POINTDA as sonn_categories_common_pointda
from datasets_pointda import CATEGORIES as pointda_categories
from datasets_pointda import CATEGORIES_COMMON_SONN as pointda_categories_common_sonn
# from datasets_mn40 import CATEGORIES_COMMON_POINTDA_SONN as pointda_categories_common_sonn
...
def run_test(args, logger, checkpoint):
    ...
    if args.dataset_type == "mn40":
        xtickslabels = sonn_categories_common #_plot_sonn
        categories_common = sonn_categories_common
    elif args.dataset_type == "modelnet10" or args.dataset_type == "shapenet10" or args.dataset_type == "scannet10":
        xtickslabels = sonn_categories_common
        categories_common = pointda_categories
    elif args.dataset_type == "mn4011":
        xtickslabels = sonn_categories_common
        categories_common = sonn_categories_common
    elif args.dataset_type == "modelnet7" or args.dataset_type == "shapenet7" or args.dataset_type == "scannet7":
        xtickslabels = pointda_categories_common_sonn
        categories_common = sonn_categories_common_pointda
    ...
    for idx_batch, data_batch in enumerate(loader):
        ...
        with torch.inference_mode():
            ...
            for p in pred:
                for i, cat in enumerate(categories_common):
                    if isinstance(cat, list) and sonn_categories[p] in cat:
                        index_pred = np.append(index_pred, i)
                    elif sonn_categories[p] == cat:
                        index_pred = np.append(index_pred, i)
        ...
        # print(torch.nn.functional.one_hot(pred, num_classes=args.n_classes_test).shape, torch.nn.functional.one_hot(target, num_classes=args.n_classes_test).shape)
