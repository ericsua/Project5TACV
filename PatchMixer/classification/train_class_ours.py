...
from PatchMixer.augmentations import center, scale1, scale2, jitter1, jitter2, rotate1, rotate2, scale_and_translate
from datasets import get_dataset
from PatchMixer.models_ours import mixer_class_singleres, mixer_class_multires
from PatchMixer.losses import loss_ce, loss_cbce, loss_lsce, smooth_loss
from PatchMixer.utils import get_logger
...
def parse_args():
    ...
    parser.add_argument('--freq_cm', type=int, default=None, help='Frequence at which plot confusion matrix')
...
def run_train(args, logger, writer):
    ...
    if args.augms is not None:
        ...
        if 'scale2' in augms_list:
            transforms_train_list.append(scale2())
            transforms_test_list.append(scale2())
    ...
    for epoch in range(args.start_epoch + 1, args.n_epochs + 1):
        ...
        if args.freq_cm is not None:
            if epoch % args.freq_cm == 0:
                plt.figure()
        ...
        # Save the last model
        if epoch == args.n_epochs or ((args.freq_save is not None) and (epoch % args.freq_save == 0)):
            torch.save({
                    'args': args,
                    'epoch': epoch,
                    'model_state_dict': model.module.state_dict() if hasattr(model, 'module') else model.state_dict(),
