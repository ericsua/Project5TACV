...
from util.scanobjectnn import ScanObjectNNHardest_mn40
from util.modelnet40 import ModelNet40Ply2048
from util.modelnet40 import ModelNet40Ply2048_sonn
from util.shapenet10 import ShapeNet10_sonn, ScanNet10_sonn, ModelNet10_sonn
...
def main_worker(rank, argss):
    ...
    if main_process():
        ...
        logger.info('#Model parameters torchinfo: {}'.format(torchinfo.summary(model, )))
        #torch.Size([8192, 4]) torch.Size([8192, 3]) torch.Size([8]) torch.Size([8192]) torch.Size([8192, 34])
        #logger.info('#Model parameters torchsummary: {}'.format(summary(model, input_size=(8192,4, 8192,3, 8, 8192, 8192,34))))
    ...
    if args.data_name == 'scanobjectnn':
    ...
    elif args.data_name == 'modelnet10sonn':
        train_transform = None
        if args.aug:
            jitter_sigma = args.get('jitter_sigma', 0.01)
            jitter_clip = args.get('jitter_clip', 0.05)
        ...
        train_data = ModelNet10_sonn(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    elif args.data_name == 'shapenet10sonn':
        train_transform = None
        if args.aug:
            jitter_sigma = args.get('jitter_sigma', 0.01)
            jitter_clip = args.get('jitter_clip', 0.05)
        ...
        train_data = ShapeNet10_sonn(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    elif args.data_name == 'scannet10sonn':
        train_transform = None
        if args.aug:
            jitter_sigma = args.get('jitter_sigma', 0.01)
            jitter_clip = args.get('jitter_clip', 0.05)
        ...
        train_data = ScanNet10_sonn(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    else:
    ...
    if args.data_name == 'scanobjectnn':
        if args.transfer:
            #val_data = ScanObjectNNHardest_mn40(data_dir=args.data_root, split=args.test_split, transform=val_transform, num_points=args.num_points) #! giusto con 1024 punti
            val_data = ScanObjectNNHardest_mn40(data_dir=args.data_root, split=args.test_split, transform=val_transform)
        else:
            val_data = ScanObjectNNHardest(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'modelnet40':
        if args.transfer:
            val_data = ModelNet40Ply2048_sonn(data_dir=args.data_root, split=args.test_split, transform=val_transform, num_points=args.num_points)
        else:
            val_data = ModelNet40Ply2048(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'modelnet10sonn':
        val_data = ModelNet10_sonn(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'shapenet10sonn':
        val_data = ShapeNet10_sonn(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'scannet10sonn':
        val_data = ScanNet10_sonn(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    else:
        raise ValueError("The dataset {} is not supported.".format(args.data_name))
    ...
    if args.model_path:
        if os.path.isfile(args.model_path):
            ...
            if args.transfer:
                print("TRANSFER")
                validateTransfer(val_loader, model, criterion)
            else:
...
def validate(val_loader, model, criterion):
    ...
    for i, (coord, feat, target) in enumerate(val_loader):
        ...
        if(i == 0):
            output_tsne = output_tsne[8:]
            
        output = output.max(1)[1] # max -> (value, index)
    ...
    plt.scatter(tsne[:, 0], tsne[:, 1], c=y_true, cmap=plt.cm.get_cmap('tab20', 40))
    plt.title('t-SNE ModelNet40 CDFormer Protocol')
    plt.savefig("plot/tnse_cdformer.png", bbox_inches='tight', dpi=300)

    conf_mat = confusion_matrix(y_true, y_pred, normalize='true') * 100 #, ModelNet40Ply2048.classes)
    conf_mat = conf_mat.astype(int)

    plt.figure(figsize=(16, 14))
    sns.heatmap(conf_mat, annot=True, fmt="d", xticklabels=ModelNet40Ply2048.classes, yticklabels=ModelNet40Ply2048.classes)
    plt.title('Confusion Matrix ModelNet40 CDFormer Protocol')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig('plot/confusion_matrix_cdformer.png',  bbox_inches='tight', dpi=300)
...
def validateTransfer(val_loader, model, criterion):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>')
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    y_true, y_pred = np.array([]), np.array([])
    #output_tsne = np.zeros((args.batch_size_val, args.classes_transfer))
    output_tsne = list()
    
    index_map = {}
    class_map = {}

    #load_class_train = importlib.import_module(f'util.{args.module_train}')
    load_class_train = getattr(sys.modules[__name__], args.class_train)
    #load_class_test = importlib.import_module(f'util.{args.module_test}')
    load_class_test =  getattr(sys.modules[__name__], args.class_test)
    #class_train = getattr(load_class_train, )
    #class_test = getattr(load_class_test, "classes")
    classes_train = getattr(load_class_train, "classes")
    classes_test = getattr(load_class_test, "classes")
    classes_train_common = getattr(load_class_train, args.classes_common_train)
    classes_test_common = getattr(load_class_test, args.classes_common_test)
    #for i, cls in enumerate(ModelNet40Ply2048.classes):
    for i, cls in enumerate(classes_train):
        #index_map[i] = index_nested(ModelNet40Ply2048.classes_commmon_pointda, cls)
        index_map[i] = index_nested(classes_train_common, cls)
        if index_map[i] != -1:
            class_map[cls] = classes_train_common[index_map[i]]
        else:
            class_map[cls] = ""
    ...
    for i, (coord, feat, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            #print("size", feat.shape, coord.shape, offset.shape, batch.shape, neighbor_idx.shape)
            output = model(feat, coord, offset, batch, neighbor_idx)
            #print("CIAOOOO")
            _, index_max = torch.max(output, dim=1)
            index_max = index_max.cpu().numpy()
            # convert predicted label to common classes label
            
            
            index_pred = list()
            index_target = list()
            
            #indices_classes_common_flat = [item for sublist in ModelNet40Ply2048.classes_common_sonn for item in sublist]
            
            #print("index_max: ", index_max)
            #print("target: ", target)
            output_common = list()
            for j, index in enumerate(index_max):
                #for z, cat in enumerate(ModelNet40Ply2048.classes_commmon_pointda): # categories common of train dataset with test dataset
                for z, cat in enumerate(classes_train_common): # categories common of train dataset with test dataset
                    #if (isinstance(cat, list) and ModelNet40Ply2048.classes[index] in cat) or (ModelNet40Ply2048.classes[index] == cat): # all categories of train dataset
                    if (isinstance(cat, list) and classes_train[index] in cat) or (classes_train[index] == cat): # all categories of train dataset
                        index_pred.append(z)
                        index_target.append(target[j].item())
                        output_common.append(output[j].cpu().numpy())
            #print("index_pred: ", index_pred)
            #print("index_target: ", index_target)
                    # elif ShapeNet10.classes[index] == cat:
                    #     index_pred.append(z)
                    #     index_target.append(target[j].item())
                    #     output_common.append(output[j].cpu().numpy())
            ...
            index_pred = torch.tensor(index_pred).cuda() # 1 predetto, [0,1,0,0,]
            index_target = torch.tensor(index_target).cuda()
            
            assert index_pred.size() == index_target.size(), "index_pred.size() != index_target.size()"
            
            if index_pred.numel() == 0:
                continue
            
            output_common = np.array(output_common)

            # pred = torch.from_numpy(index_pred).to(int).to(args.device)
            # index_max = [ModelNet40Ply2048.classes_common_sonn[i] for i in index_max]
            # index_max = np.array(index_max)
            # index_max = torch.from_numpy(index_max).cuda()
            #loss = criterion(output, index_max)
            #loss = criterion(output, target)
            #loss = criterion(index_pred.unsqueeze(0), index_target.unsqueeze(0))
            
                
            #output_filtered = output_common
            #output_filtered = np.zeros((args.batch_size_val, args.classes_transfer))
            output_filtered = np.zeros((output_common.shape[0], args.classes_transfer))
            #for i, item in enumerate(output.cpu().numpy()):
            #print("output_common: ", output_common.shape)
            for o, item in enumerate(output_common):
                for j, logit in enumerate(item):
                    index = index_map[j]
                    if index != -1:
                        output_filtered[o][index] += logit
                    
            #print("output_filtered: ", output_filtered.shape)
            
            output_tsne.extend(output_filtered)
            # output_tsne = np.vstack((output_tsne, output_filtered))
            # if(i == 0):
            #     output_tsne = output_tsne[args.batch_size_val:]
        #output = output.max(1)[1]
        #output_tsne = np.array(output_tsne)
        output = index_pred
        target = index_target
        
        #print("output tsne: ", output_tsne.shape)
       
        y_true = np.append(y_true, target.cpu().numpy())
        y_pred = np.append(y_pred, output.cpu().numpy())
        
        # print("target: ", target.cpu().numpy().shape, target)
        # print("output: ", output.cpu().numpy().shape, output)
        # print("ytrue: ", y_true.shape, y_true)
        # print("ypred: ", y_pred.shape, y_pred)
        ...
        intersection, union, target = intersectionAndUnionGPU(output, target, args.classes_transfer, args.ignore_label)
        # if args.multiprocessing_distributed:
        #     dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(target)
        intersection, union, target = intersection.cpu().numpy(), union.cpu().numpy(), target.cpu().numpy()
        intersection_meter.update(intersection), union_meter.update(union), target_meter.update(target)
    ...
    print("ACC SKLEARN: ", accuracy_score(y_true, y_pred))
            
    tsne = TSNE(n_components=2, random_state=42).fit_transform(output_tsne, y=y_true)

    plt.figure(figsize=(16, 12))
    scatter = plt.scatter(tsne[:, 0], tsne[:, 1], c=y_pred, cmap=plt.cm.get_cmap('tab20', args.classes_transfer))
    legend2 = plt.legend(handles=scatter.legend_elements()[0], labels=classes_test_common, bbox_to_anchor=(1.04, 1), loc="upper left", title='Classes')
    plt.gca().add_artist(legend2)
    plt.title(f't-SNE {args.data_trained_plots} {args.protocol} Protocol - tested on {args.data_tested_plots}')
    plt.savefig(f"plot/tnse_CDFormer_{args.data_trained_path}_{args.data_tested_path}_{args.protocol.lower()}.png", bbox_inches='tight', dpi=300)

    conf_mat = confusion_matrix(y_true, y_pred, normalize='true') * 100 #, ModelNet40Ply2048.classes)
    conf_mat = conf_mat.astype(int)

    plt.figure(figsize=(16, 14))
    #sns.heatmap(conf_mat, annot=True, fmt="d", xticklabels=ModelNet40Ply2048.classes_commmon_pointda, yticklabels=ShapeNet10.classes)
    sns.heatmap(conf_mat, annot=True, fmt="d", xticklabels=classes_train_common, yticklabels=classes_test_common)
    plt.title(f'Confusion Matrix {args.data_trained_plots} {args.protocol} Protocol - tested on {args.data_tested_plots}')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig(f"plot/cm_CDFormer_{args.data_trained_path}_{args.data_tested_path}_{args.protocol.lower()}.png", bbox_inches='tight', dpi=300)
    ...
    if main_process():
        logger.info('Val result: allAcc/mAcc {:.3f}/{:.3f}.'.format(allAcc, mAcc))
        for i in range(args.classes_transfer):
            logger.info('Class_{} Result: accuracy {:.4f}.'.format(i, accuracy_class[i]))
        logger.info('<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<')

    return 0, mIoU, mAcc, allAcc
