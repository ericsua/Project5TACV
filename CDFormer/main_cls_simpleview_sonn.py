...
def validate(val_loader, model, criterion):
    ...
    plt.scatter(tsne[:, 0], tsne[:, 1], c=y_true, cmap=plt.cm.get_cmap('tab20', 15))
    #plt.title('t-SNE ScanObjectNN CDFormer Protocol')
    plt.title('t-SNE ScanObjectNN SimpleView Protocol')
    plt.savefig("plot/cdformer_tsne_sonn_sonn_simpleview.png", bbox_inches='tight', dpi=300)
    #plt.savefig("plot/cdformer_tsne_sonn_sonn_cdformer.png", bbox_inches='tight', dpi=300)

    conf_mat = confusion_matrix(y_true, y_pred, normalize='true') * 100 #, ModelNet40Ply2048.classes)
    conf_mat = conf_mat.astype(int)

    plt.figure(figsize=(16, 14))
    sns.heatmap(conf_mat, annot=True, fmt="d", xticklabels=ScanObjectNNHardest.classes, yticklabels=ScanObjectNNHardest.classes)
    plt.title('Confusion Matrix ScanObjectNN SimpleView Protocol')
    #plt.title('Confusion Matrix ScanObjectNN CDFormer Protocol')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig('plot/cdformer_cm_sonn_sonn_simpleview.png',  bbox_inches='tight', dpi=300)
    #plt.savefig('plot/cdformer_cm_sonn_sonn_cdformer.png',  bbox_inches='tight', dpi=300)
