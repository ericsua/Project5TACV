...
from util.shapenet10 import ModelNet10
from util.shapenet10 import ScanNet10
from util.graspnet import GraspNetPC10
...
def main_worker(rank, argss):
    ...
    if args.data_name == 'scanobjectnn':
    ...
    elif args.data_name == 'shapenet10':
        train_transform = None
        if args.aug:
            scale_ratio = args.get('scale_ratio', 0.5)
            shift_range = args.get('shift_range', 0.2)
            train_transform = transform.Compose([
                transform.RandomDiffScale(scale_low=1/(1+scale_ratio), scale_high=1+scale_ratio),
                transform.RandomShift(shift_range=shift_range),
            ])
        train_data = ShapeNet10(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    elif args.data_name == 'modelnet10':
        train_transform = None
        if args.aug:
            scale_ratio = args.get('scale_ratio', 0.5)
            shift_range = args.get('shift_range', 0.2)
        ...
        train_data = ModelNet10(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    elif args.data_name == 'scannet10':
        train_transform = None
        if args.aug:
            scale_ratio = args.get('scale_ratio', 0.5)
            shift_range = args.get('shift_range', 0.2)
        ...
        train_data = ScanNet10(data_dir=args.data_root, split=args.train_split, transform=train_transform)
    elif args.data_name == 'graspnet':
        train_transform = None
        if args.aug:
            scale_ratio = args.get('scale_ratio', 0.5)
            shift_range = args.get('shift_range', 0.2)
        ...
        train_data = GraspNetPC10(data_dir=args.data_root, split=args.train_split, transform=train_transform, mode=args.mode, sensor=args.sensor)
    else:
    ...
    if args.data_name == 'scanobjectnn':
        ...
        val_data = ShapeNet10(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'modelnet10':
        val_data = ModelNet10(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'scannet10':
        val_data = ScanNet10(data_dir=args.data_root, split=args.test_split, transform=val_transform)
    elif args.data_name == 'graspnet':
        val_data = GraspNetPC10(data_dir=args.data_root, split=args.test_split, transform=val_transform, mode=args.mode, sensor=args.sensor)
    else:
        raise ValueError("The dataset {} is not supported.".format(args.data_name))
